...
const userIdSchema: ParamSchema = {
  custom: {
    options: async (value: string, { req }) => {
      // check value có phải ObjectId hay ko?
      if (!ObjectId.isValid(value)) {
        throw new ErrorWithStatus({
          message: USERS_MESSAGES.INVALID_USER_ID, //trong message.ts thêm INVALID_FOLLOWED_USER_ID: 'Followed user id is invalid'
          status: HTTP_STATUS.NOT_FOUND
        })
      }
      // vào database tìm user đó xem có ko?
      const user = await databaseService.users.findOne({ _id: new ObjectId(value) })
      if (user === null) {
        throw new ErrorWithStatus({
          message: USERS_MESSAGES.USER_NOT_FOUND, //trong message.ts thêm FOLLOW_USER_NOT_FOUND: 'Followed user not found'
          status: HTTP_STATUS.NOT_FOUND
        })
      }
      ...
      Authorization: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            ...
            // nếu ko có access_token thì ném lỗi 401
            if (!accessToken) {
              throw new ErrorWithStatus({
          ...
          options: async (value: string, { req }) => {
            ...
            try {
              const [decoded_refresh_token, refresh_token] = await Promise.all([
                ...
                // tìm refresh_token này có tồn tại trong database hay ko?
  ...
  checkSchema(
    {
      email_verify_token: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            // ktra người dùng có truyền email_verify_token lên hay ko?
            if (!value) {
              throw new ErrorWithStatus({
            ...
            try {
              ...
              // sau khi verify thành công thì đc payload của email_verify_token: decoded_email_verify_token
  ...
  checkSchema(
    {
      forgot_password_token: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            // ktra người dùng có truyền forgot_password_token lên hay ko?
            if (!value) {
              throw new ErrorWithStatus({
            ...
            try {
              ...
              // sau khi verify thành công thì đc payload của forgot_password_token: decoded_forgot_password_token
      ...
      username: {
        ...
        custom: {
          options: async (value, { req }) => {
            ...
            // tìm xem có username này trong database hay chưa?
            const user = await databaseService.users.findOne({ username: value })
            // nếu có rồi thì ném lỗi
            if (user) {
